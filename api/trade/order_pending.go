...
func OrderPendingHttp(c *gin.Context) {
	api.DoHttpProcess(new(OrderPendingApi), c)
}

type OrderPendingApi struct {
	api.Base

	apiParams model.OrdersPendingReq
}
...
func (a *OrderPendingApi) ProcessHttp() {
	...
	res, err := new(service.TradeSvr).OrdersPending(a.apiParams)
	if err != nil {
